...
(() => {
  ...
  // rules.json xoá cookie ở mọi request main-frame tới Studocu — kể cả
  // cf_clearance. Nếu mở tài liệu khi chưa từng xác minh Cloudflare,
  // trang "Just a moment..." lặp vĩnh viễn: mỗi lần xác minh xong tải
  // lại trang thì request lại không mang cookie clearance.
  // Cách xử lý: phát hiện trang xác minh → báo background TẠM TẮT bypass
  // để xác minh hoàn tất → trang tài liệu mở được → bật bypass lại và
  // reload đúng một lần để áp dụng.
  const CF_TITLE_RE = /just a moment|attention required|checking (your|the) browser|verify (you are|you're) human|cloudflare/i;
  const CF_BODY_RE = /cf-browser-verification|challenge-platform|challenge-running|challenge-stage|cf-turnstile/i;
  const CF_RELOAD_FLAG = 'docunchain-cf-reloaded';

  function isCloudflareChallengePage() {
    // Trang Studocu thật luôn có #__NEXT_DATA__ hoặc trang .pf đã render.
    if (document.getElementById('__NEXT_DATA__') || document.querySelector('.pf')) return false;
    if (CF_TITLE_RE.test(document.title || '')) return true;
    const html = (document.documentElement ? document.documentElement.innerHTML : '').slice(0, 40000);
    return CF_BODY_RE.test(html);
  }

  function notifyBackground(action) {
    try {
      return new Promise(resolve => {
        chrome.runtime.sendMessage({ action }, res => { void chrome.runtime.lastError; resolve(res); });
      });
    } catch (e) { return Promise.resolve(null); }
  }

  function reportChallengePassed() {
    notifyBackground('CF_CHALLENGE_PASSED').then(res => {
      // Chỉ tự reload một lần mỗi phiên tab để áp dụng bypass vừa bật lại —
      // tránh vòng lặp reload nếu Cloudflare tiếp tục thách thức.
      if (res && res.resumed && !sessionStorage.getItem(CF_RELOAD_FLAG)) {
        sessionStorage.setItem(CF_RELOAD_FLAG, '1');
        location.reload();
      }
    });
  }

  function watchCloudflareChallenge() {
    notifyBackground('CF_CHALLENGE_DETECTED');
    const t = setInterval(() => {
      if (!isCloudflareChallengePage()) {
        clearInterval(t);
        reportChallengePassed();
        return;
      }
      // Heartbeat: background gia hạn bộ đếm an toàn khi còn trang xác minh.
      notifyBackground('CF_CHALLENGE_DETECTED');
    }, 2000);
  }

  // Kiểm tra lặp khi khởi động: lúc document_start trang xác minh chưa
  // render xong; khi nhận diện được thì chuyển sang chế độ theo dõi.
  let cfChecks = 0;
  const cfStartup = setInterval(() => {
    cfChecks++;
    if (isCloudflareChallengePage()) {
      clearInterval(cfStartup);
      watchCloudflareChallenge();
    } else if (document.getElementById('__NEXT_DATA__') || document.querySelector('.pf') || cfChecks >= 15) {
      clearInterval(cfStartup);
      reportChallengePassed();
    }
  }, 1200);
